...
BOOST_AUTO_TEST_SUITE(line_refinement)
BOOST_AUTO_TEST_CASE(lr_insert_knot)
{
  ...
  for (size_t i = 0; i < nurbs.cpi().size(); ++i) {
    ...
    BOOST_CHECK_CLOSE(p.x(), i * 1.0f, 0.00001f);
    BOOST_CHECK_CLOSE(p.y(), 0.0f, 0.00001f);
    BOOST_CHECK_CLOSE(p.z(), 0.0f, 0.00001f);
    BOOST_CHECK_CLOSE(p.w(), 1.0f, 0.00001f);
  }

}

BOOST_AUTO_TEST_CASE(lr_insert_knot_multiple_spans)
{
  using namespace squanch;
  ...
  for (size_t i = 0; i < nurbs.cpi().size(); ++i) {
    auto&& p = model.get_point(nurbs.cpi()(i));
    BOOST_CHECK_CLOSE(p.x(), i * 1.25, 0.0000001);
    BOOST_CHECK_CLOSE(p.y(), 0.0, 0.0000001);
    BOOST_CHECK_CLOSE(p.z(), 0.0, 0.0000001);
    BOOST_CHECK_CLOSE(p.w(), 1.0, 0.0000001);
  }
}

BOOST_AUTO_TEST_CASE(line_h_refine_elements)
{
  using namespace squanch;
  squanch::Model<double> model;
  ...
  BOOST_CHECK_EQUAL(nurbs.curves()[0].knots().size(), 6);
  BOOST_CHECK_EQUAL(nurbs.curves()[0].n(), 4);

}

BOOST_AUTO_TEST_CASE(p_refine_line)
{
  using namespace squanch;
  squanch::Model<double> model;
  ...
  auto check_line = [](const squanch::Model<double>& model, Nurbs<double, 1>& nrb)
  {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      BOOST_CHECK_CLOSE(p.x(), 10.0 * i * du, 0.0000001);
      BOOST_CHECK_CLOSE(p.y(), 0.0, 0.0000001);
      BOOST_CHECK_CLOSE(p.z(), 0.0, 0.0000001);
      BOOST_CHECK_CLOSE(p.w(), 1.0, 0.0000001);
    }
...
BOOST_AUTO_TEST_CASE(lr_h_refine_circle)
{
  using namespace squanch;
  squanch::Model<double> model;
...
BOOST_AUTO_TEST_CASE(p_refine_circle)
{
  using namespace squanch;
...
BOOST_AUTO_TEST_CASE(h_p_refine_rectangle)
{
  using namespace squanch;
  ...
  auto check_rect = [](const squanch::Model<double>& model, Nurbs<double, 2>& nrb)
  {
    ...
    for (size_t i = 0; i < 11; ++i) {
      ...
      for (size_t j = 0; j < 11; ++j) {
        ...
        BOOST_CHECK_CLOSE( p.x(), 5.0 + u[0]*5, 0.00000001);
        BOOST_CHECK_CLOSE( p.y(), u[1]*10, 0.00000001);
        BOOST_CHECK_CLOSE( p.z(), 0, 0.00000001);
        BOOST_CHECK_CLOSE( p.w(), 1, 0.00000001);
      }
    }
  };
...
BOOST_AUTO_TEST_CASE(h_p_refine_cube)
{
  using namespace squanch;
  using namespace Eigen;
  // create a line
    ...
    for (size_t i = 0; i < 11; ++i) {
      ...
      for (size_t j = 0; j < 11; ++j) {
        ...
        for (size_t k = 0; k < 11; ++k) {
          ...
          BOOST_CHECK_CLOSE( p.x(), 5.0 + u[0]*5, 0.00000001);
          BOOST_CHECK_CLOSE( p.y(), u[1]*10, 0.00000001);
          BOOST_CHECK_CLOSE( p.z(), u[2]*7.5, 0.00000001);
          BOOST_CHECK_CLOSE( p.w(), 1, 0.00000001);
        }
      }
    }
...
BOOST_AUTO_TEST_CASE(ThreeDCurvedBeamRefinement)
{
  using namespace squanch;
