...
BOOST_AUTO_TEST_SUITE(pointInversion)
...
BOOST_AUTO_TEST_CASE(line) {
  ...
  for (auto i = 0; i < num_points; ++i) {
    ...
    BOOST_CHECK_CLOSE(u, u_calculated, 0.0000001);
  }
}
...
BOOST_AUTO_TEST_CASE(circle) {
  ...
  for (auto i = 0; i < num_points; ++i) {
    ...
    BOOST_CHECK_CLOSE(u_real[i], u_calculated, 0.0001);
  }
}

BOOST_AUTO_TEST_CASE(rectangular_surface_1d)
{
  using namespace squanch;
  squanch::Model<double> model;
...
BOOST_AUTO_TEST_CASE(rectangular_surface_2d)
{
  using namespace squanch;
  squanch::Model<double> model;
  ...
  for (auto i = 0; i < num_points; ++i) {
    ...
    for (auto j = 0u; j < num_points; ++j) {
      ...
      BOOST_CHECK_CLOSE(u[0], u_calculated[0], 0.0000001);
      BOOST_CHECK_CLOSE(u[1], u_calculated[1], 0.0000001);
    }
    u[1] = 0.0;
    p[1] = 0.0;
  }
  ...
  for (auto i = 0; i < num_points; ++i) {
    ...
    for (auto j = 0u; j < u_real.size(); ++j) {
      ...
      if (u[0] == 0) {
        ...
        BOOST_CHECK_CLOSE(u[0] + 0.1, u_calculated[0] + 0.1, 0.0001);
      }
      else {
        BOOST_CHECK_CLOSE(u[0], u_calculated[0], 0.0001);
      }
      BOOST_CHECK_CLOSE(u[1], u_calculated[1], 0.0001);
    }
  }
}
